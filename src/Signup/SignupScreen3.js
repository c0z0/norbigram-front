...
export default class SignupScreen3 extends Component {
	constructor() {
		super();
		this.state = {disabled: true, err: false};
	...
	_onPress() {
		if (!this.state.disabled) {
			...
			axios.post('http://localhost:3000/u/username', {username: this.text})
				.then((res) => {
					this.setState({loading: false, err: false})
					if (res.data.foundUser)
						return this.setState({err: "Username already in use"})
					this.setState({loading: true})
					axios.post('http://localhost:3000/u/register', 
						{
							user: {
							...
							this.setState({loading: false, err: false});
							ToastAndroid.show(res.data.newUser.name, ToastAndroid.LONG)
						}, (err) => {
		...
		return (
			<View style={styles.containerExt}>
				...
				<View style={styles.container}>
					<View>
						<Icon name="user-circle" size={130} color="#555"/>
					</View>
					<TextInput
						autoFocus={true}
...
const DisabledLogin = (props) => {
	...
	let text = <Text style={stylesLoc.text}>Sign Up</Text>;
